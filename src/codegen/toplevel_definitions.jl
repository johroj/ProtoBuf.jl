...
function translate(io, rp::ResolvedProtoFile, file_map::Dict{String,ResolvedProtoFile}, options)
    ...
    if !is_namespaced(p)
        ...
        for path in import_paths(p)
            ...
            if !is_namespaced(dependency)
                ...
                println(io, "include(", _include_format(proto_script_name(dependency)), ")")
                ...
                println(io, "include(", _include_format(namespaced_top_include(dependency)), ")")
    ...
    if options.parametrize_oneofs
        for name in _non_cyclic_defs(p)
